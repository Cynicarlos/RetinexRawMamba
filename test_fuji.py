    ...
    with open('Fuji_results.txt', 'a') as f:
        for idx, data in enumerate(tqdm_loader):
            ...
            if save_image:
                ...
                save_path = os.path.join(save_dir, os.path.basename(input_path)[:-4]) + '.png'
...
@torch.no_grad()
def merge_test(model, dataloader):
    model.eval()
    psnr_sum = 0.0
    ssim_sum = 0.0
    lpips_sum = 0.0
    ...
    with open('Fuji_merge_results.txt', 'a') as f:
        for idx, data in enumerate(tqdm_loader):
            input_path, gt_path, ratio = data['input_path'][0], data['gt_path'][0], data['ratio'][0]
...
if __name__ == "__main__":
    ...
    model = model.cuda()
    checkpoint = torch.load('fuji_best_model.pth')
